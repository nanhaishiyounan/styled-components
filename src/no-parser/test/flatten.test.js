...
describe('preparsed flatten without executionContext', () => {
  ...
  it('drops nulls', () => {
    ...
    expect(flatten([['foo', false, 'bar', undefined, 'baz', null]])).toEqual([
      ['foo', 'bar', 'baz'],
    ])
  })
  ...
  it('hypenates objects', () => {
    ...
    expect(flatten([['some:thing;', obj, 'something: else;']])).toEqual([
      ['some:thing;', css, 'something: else;'],
    ])
  })

  it('flattens nested rulesets', () => {
    expect(flatten([['a', [['c']], 'b']])).toEqual([['a', 'b'], ['c']])
  })

  it('flattens double nested rulesets', () => {
    expect(flatten([['a', [['c', [['d']]]], 'b']])).toEqual([
      ['a', 'b'],
      ['c'],
      ['d'],
    ])
...
describe('preparsed flatten with executionContext', () => {
  ...
  it('drops nulls', () => {
    // $FlowInvalidInputTest
    expect(
      flatten([['foo', false, 'bar', undefined, 'baz', null]], {})
    ).toEqual(['foobarbaz'])
  })
  ...
  it('flattens nested rulesets', () => {
    expect(flatten([['a', [['c']], 'b']], {})).toEqual(['ab', 'c'])
  })

  it('flattens double nested rulesets', () => {
    expect(
      flatten([['a', [['c', 'd', [['e', 'f'], ['g', 'h']]]], 'b']], {})
    ).toEqual(['ab', 'cd', 'ef', 'gh'])
  })

  it('flattens subarrays', () => {
    expect(flatten([[1, 2, [3, 4, 5], 'come:on;', 'lets:ride;']], {})).toEqual([
      '12345come:on;lets:ride;',
    ])
  })
  ...
  it('resolves rulesets after executing functions', () => {
    ...
    expect(flatten([['foo', func, 'baz']], {})).toEqual([
      'foobaz',
      'add me to the end',
    ])
  })
